...
bowtie2 -x $GENOME_INDEX -1 $F_FILE -2 $R_FILE -S "$SAM_FILE".sam -p 8
...
samtools tview "$SAM_FILE".bam $GENOME_FILE
...
mkdir -p $CUR_PATH/assembly/ls_contigs/$ORGANISM2/$STRAIN2
cp "$SAM_FILE"* $CUR_PATH/assembly/ls_contigs/$ORGANISM2/$STRAIN2/.
cp "$STRAIN2"_ls_contigs.fa $CUR_PATH/assembly/ls_contigs/$ORGANISM2/$STRAIN2/.
rm -r $WORK_DIR/